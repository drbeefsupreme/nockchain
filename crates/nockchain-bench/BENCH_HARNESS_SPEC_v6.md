# Spec: Trustworthy SOL Benchmark Harness v6

## Status

This spec supersedes `BENCH_HARNESS_SPEC.md`, `BENCH_HARNESS_SPEC_v3.md`,
`BENCH_HARNESS_SPEC_v4.md`, and `BENCH_HARNESS_SPEC_v5.md`.

v6 preserves the unified orchestration architecture from v5 (shared
orchestrator with backend adapters) but revises it against the Phase 0/1
implementation experience:

- clarifies that the orchestrator extraction happens at the Phase 2 boundary,
  not during Phase 1 where only one backend exists
- specifies the backend adapter contract as a Rust trait with concrete
  method signatures
- resolves provenance ownership ambiguity between orchestrator and backend
- aligns `RequestedCase` with fields the implementation actually needed
- adds a "refactor first" gate at the start of Phase 2

Changes from v5 are marked with `[v6]` in the margin where substantive.
...
├── orchestrate.rs   # shared trusted orchestration pipeline       [v6: Phase 2+]
...
`[v6]` `orchestrate.rs` is introduced at the Phase 2 boundary by extracting
the orchestration loop from `native.rs`. During Phase 1, the orchestration
logic lives inline in `native.rs` because there is only one backend and the
abstraction has no second consumer. See Section 19 for the extraction protocol.
...
`[v6]` **Implementation timing:** During Phase 1, the orchestration logic lives
inline in `native.rs` as `execute_native_trusted_run()`. This is acceptable
because Phase 1 has only one backend and extracting a backend-agnostic
orchestrator with a single consumer would be premature abstraction. The
orchestrator is extracted into `orchestrate.rs` at the start of Phase 2, when
the Docker backend provides the second consumer that motivates the abstraction.
See Section 19 Phase 2 step 1.

### 8.4 Backend Adapter Contract

`[v6]` Each backend adapter must implement the following trait:

```rust
/// Backend-specific execution adapter for the shared trusted orchestrator.
///
/// The orchestrator calls these methods in order:
///   1. prepare() — once per benchmark, before any runs
///   2. execute_run() — once per warmup or measured run
///   3. capture_provenance() — once, after all runs complete
///   4. capture_raw_evidence() — once, for backend-specific raw artifacts
///   5. cleanup() — once, regardless of success or failure
pub trait BenchBackend {
    /// Prepare the runtime environment (e.g., create container, set up dirs).
    /// Called once before the first run. Returns backend-specific state
    /// that will be passed to execute_run().
    async fn prepare(&mut self, resolved: &ResolvedCase, output_root: &Path)
        -> Result<()>;

    /// Execute one run of the resolved case. The orchestrator supplies the
    /// run directory (e.g., "runs/run-0") and run_id. The backend invokes
    /// the once-run engine (directly for native, via sol run-once for Docker)
    /// and writes per-run artifacts to run_dir.
    ...
    /// Return backend-specific provenance fields. The orchestrator merges
    /// these with shared provenance (host identity, git identity, fixture
    /// identity) and writes the combined provenance.json.
    fn capture_provenance(&self) -> Result<BackendProvenance>;

    /// Write backend-specific raw evidence to the raw/ subdirectory
    /// (e.g., docker_inspect.json, docker_info.json, container_env.json).
    /// Native backend may write nothing here beyond host_env.json.
    async fn capture_raw_evidence(&self, raw_dir: &Path) -> Result<()>;

    /// Clean up backend-owned resources (e.g., remove containers, volumes).
    /// Called in all cases, including after failures.
...
`[v6]` `BackendProvenance` is an enum that carries either native or Docker
provenance fields:

```rust
pub enum BackendProvenance {
    Native,  // no additional fields beyond shared provenance
    Docker {
...
The orchestrator merges `BackendProvenance` with shared fields (host identity,
git identity, fixture identity) to produce the final `provenance.json`.
...
`[v6]` The implementation may add runner-specific configuration fields to
`RequestedCase` beyond the ones listed above, provided they are needed by
`SolBenchRunner` or `SolBenchConfig`. Examples from Phase 1 implementation:
`checkpoint_recovery_timeout_ms`, `checkpoint_recovery_tolerance_pct`,
`gc_drop_threshold_mib`, `page_fault_minor_burst_threshold`,
`page_fault_major_burst_threshold`. These are considered implementation detail
and do not require a spec revision to add.
...
`[v6]` Provenance is a collaboration between the orchestrator and the backend
adapter. The orchestrator captures shared fields and the backend returns
`BackendProvenance` (see Section 8.4). The orchestrator merges them and writes
the combined `provenance.json`.
...
Backend-contributed fields for Docker mode:
...
- orchestrator-owned:
  ...
  - `requested_case.json`
  - `resolved_case.json`
  - `provenance.json` (merged from shared + backend contributions)
  - `summary.json`
  - `verdict.json`
- backend-owned additions:
  - backend-specific provenance fields (contributed via `capture_provenance()`)
  - backend-specific raw evidence under `raw/` (written by `capture_raw_evidence()`)
...
- host provenance capture (via `BackendProvenance::Native`)
- direct invocation of the once-run engine
- writing `raw/host_env.json`
...
`[v6]` Phase 1 implements the once-run engine and the full orchestration loop,
but the orchestration lives inline in `native.rs`. This is intentional: with
only one backend, extracting a backend-agnostic orchestrator would be premature
abstraction. The Phase 1 code is written so that it can be cleanly split at the
Phase 2 boundary.
...
- the orchestration logic (warmup/measured loop, cooldown, summary, verdict)
  is structured so it can be extracted into `orchestrate.rs` at the Phase 2
  boundary without rewriting the native backend

### Phase 2: Docker Backend On The Shared Orchestrator

`[v6]` Phase 2 begins with a refactoring gate before any new Docker code:

**Step 0: Extract shared orchestrator.** Move the orchestration loop from
`native.rs` into `orchestrate.rs`. Define the `BenchBackend` trait (Section
8.4). Convert the existing native code into a `NativeBackend` that implements
the trait. Verify that `sol bench` native mode still produces identical
artifacts after this refactoring. This step must be completed and verified
before any Docker implementation begins.
...
3. add Docker variant to `ExecutionRequest`
4. implement `DockerBackend` that satisfies the `BenchBackend` trait
5. add host/container provenance capture (dual binary identity via
   `BackendProvenance::Docker`)
6. add concurrent Docker stats API polling to `container_samples.ndjson`
7. add host/container version skew check
8. support explicit work dir modes
9. capture `raw/docker_inspect.json`, `raw/docker_info.json`, and
   `raw/container_env.json`

Exit criteria:
- Docker `sol bench` executes replay inside the container via `sol run-once`
- Docker and native trusted runs both use the same shared orchestrator and
  `BenchBackend` trait
...
- `sol bench` native mode produces identical artifacts to pre-refactoring
  (verified at step 0)
...
7. `sol bench` native and Docker modes share one trusted orchestration
   contract via the `BenchBackend` trait.
