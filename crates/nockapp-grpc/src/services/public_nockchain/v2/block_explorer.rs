...
impl BlockExplorerCache {
    ...
    async fn initialize_inner(self: Arc<Self>, handle: Arc<dyn BalanceHandle>) -> GrpcResult<bool> {
        ...
        if first_start > 0 {
            info!(
                resume_height = first_start - 1,
                "Starting block explorer cache backfill task"
            );
            let cache = Arc::clone(&self);
            let handle_clone = Arc::clone(&handle);
            tokio::spawn(async move {
                if let Err(err) = cache.backfill_older(handle_clone, first_start - 1).await {
    ...
    async fn backfill_older(
        ...
        loop {
            ...
            let chunk = match self.peek_blocks_range(&handle, start, upper).await {
                ...
                Err(NockAppGrpcError::PeekReturnedNoData) => {
                    debug!(
                        "No data for heaviest-chain range {}-{} during backfill",
                        start, upper
                    );
                    if start == 0 {
                        break;
