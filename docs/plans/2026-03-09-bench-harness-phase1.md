...
**Goal:** Implement the BENCH_HARNESS_SPEC_v4.md Phase 1 native trustworthy SOL harness with a shared once-run core, native `sol bench`, persisted artifacts, summary statistics, verdict computation, and a `sol quick-bench` refactor onto the same execution seam.

**Architecture:** Add a Phase 1 `speed_of_light::harness` library surface that owns requested/resolved case modeling, provenance capture, artifact writing, summary math, and native run orchestration. Reuse `SolBenchRunner` as the measurement engine via one shared once-run function so `sol bench` and `sol quick-bench` stay behaviorally aligned.
...
### Task 3: Add native trusted `sol bench` orchestration and refactor `sol quick-bench`
...
- `sol bench` parses with Phase 1 native trusted options
- non-release trusted runs are rejected by default
- measured runs below the Phase 1 minimum are rejected
- `sol quick-bench` still parses and routes through the shared replay config path

**Step 2: Run test to verify it fails**

Run: `cargo test -p nockchain-bench --release sol_run_cli`

Expected: FAIL because `sol bench` and its policy checks do not exist yet.

**Step 3: Write minimal implementation**

Add the `sol bench` subcommand, native repetition loop, cooldown handling, artifact-tree root writing, release gating, and the `sol quick-bench` refactor onto the shared once-run seam.
