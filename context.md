...
Below are some instructions recently given to another LLM. They implemented
Option 1 and got the test `persist_event_to_pma_copies_and_resets` passing.

Your objective: Validate the design for Option 1, making sure to take a critical
eye. If it looks good, we are going to move onto a more thorough integration test.

## Option 1 Validation Notes (2026-01-06)

**Status: Design looks sound. Ready for integration testing.**

All three tests pass:
- `persist_event_to_pma_copies_and_resets`
- `persist_event_to_pma_multiple_events`
- `persist_event_to_pma_fallback_without_pma`

### Minor Issues to Address

1. **Misleading drop-order comment** (`form.rs:745-746`): Comment says guard must be
   declared after `pma` so it's dropped first, but Rust drops in declaration order,
   so `pma` drops before `_pma_guard`. This happens to work because `PmaInstallGuard`
   only clears a thread-local (doesn't reference `Pma`), but the comment is confusing.

2. **Arc clone TODO** (`pma.rs:300-302`): The `Arc::clone(pma.arena())` has a "TODO
   not sure this is right" comment. It IS correct (avoids borrow conflict during
   mutation), but adds overhead. Consider passing arena separately for performance.

3. **`test_jets` not copied**: `Vec<NounSlab>` doesn't impl `PmaCopy`. Acknowledged
   in code comments. Fine for testing but document this limitation.

4. **Unnecessary `scry_stack` copy**: `scry_stack` is reset to `D(0)` in `event_update`
   before `persist_event_to_pma` is called, so copying it is benign but wasteful.

### Recommendations for Integration Test

1. **Boot an actual kernel**: Current tests use `dummy_serf()` with `arvo = D(0)`.
   Need to test with real kernel (e.g., dumb.jam) running poke/peek cycles.

2. **Memory pressure test**: Run 10+ events with increasing state size.

3. **Complex nested modification**: Test a poke that modifies part of a deeply
   nested structure where some subtrees remain in PMA and others are new on stack.

4. **Debug assertion**: Add verification that no stack pointers leak into PMA
   (similar to existing `debug_assert_offsets`).

5. **PMA sizing**: Test PMA uses 100K words. Real kernels need much more.
